...
const MyJobs = () => {
  const { loading, error, myJobs, message, fetched } = useSelector(
    (state) => state.jobs
  );
  const dispatch = useDispatch();

  // ✅ Handle errors and messages separately to avoid unnecessary re-renders
  useEffect(() => {
    if (error) {
      toast.error(error);
  ...
  // ✅ Fetch jobs only when `fetched` is false
  useEffect(() => {
    if (!fetched) {
          ...
          <div className="applications_container">
            {myJobs.map((element) => (
              <div className="card" key={element._id}>
                ...
                <p className="sub-sec"><span>Job Niche:</span> {element.jobNiche}</p>
                ...
                <p className="sub-sec"><span>Qualifications:</span> {element.qualifications}</p>
